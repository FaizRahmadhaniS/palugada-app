...
function Sidebar({ user, onLogout, isOpen, onClose }: { user:any; onLogout:()=>void; isOpen:boolean; onClose:()=>void }) {
      ...
      <div style={{ display:'flex', alignItems:'center', height:64, padding:'0 16px', borderBottom:`1px solid ${border}`, gap:10, flexShrink:0, justifyContent: col && !mob ? 'center' : 'space-between' }}>
        {(!col || mob) ? (
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <div style={{ width:32, height:32, borderRadius:10, background:GREEN, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, boxShadow:'0 4px 12px rgba(5,150,105,.28)' }}>
              <span style={{ color:'#fff', fontWeight:900, fontSize:14 }}>P</span>
            </div>
            <div>
              <p style={{ fontSize:13, fontWeight:700, color: isDark?'#fff':'#111827', margin:0 }}>Palugada</p>
              <p style={{ fontSize:10, color:'#10b981', fontWeight:600, margin:0, textTransform:'capitalize' }}>{user?.role}</p>
            </div>
          </div>
        ) : (
          <div style={{ width:32, height:32, borderRadius:10, background:GREEN, display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 4px 12px rgba(5,150,105,.28)' }}>
            <span style={{ color:'#fff', fontWeight:900, fontSize:14 }}>P</span>
          </div>
        )}
  ...
  return (
    <>
      ...
      <div className="hidden lg:block" style={{ height:'100vh', position:'sticky', top:0, flexShrink:0, zIndex:30 }} onMouseEnter={enter} onMouseLeave={leave}>
        {body(false)}
      </div>
      {/* Mobile overlay */}
      <AnimatePresence>
        {isOpen && (
          <>
            <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
              onClick={onClose} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.5)', backdropFilter:'blur(4px)', zIndex:40 }} className="lg:hidden" />
            <motion.div initial={{ x:'-100%' }} animate={{ x:0 }} exit={{ x:'-100%' }}
              transition={{ type:'spring', damping:28, stiffness:220 }}
              style={{ position:'fixed', top:0, left:0, bottom:0, zIndex:50, boxShadow:'4px 0 20px rgba(0,0,0,.15)' }} className="lg:hidden">
              {body(true)}
            </motion.div>
          </>
        )}
  ...
  return (
    <header style={{ height:64, display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 24px', position:'sticky', top:0, zIndex:20, borderBottom:`1px solid ${border}`, background:bg, backdropFilter:'blur(16px)', flexShrink:0 }}>
      <div style={{ display:'flex', alignItems:'center', gap:12, minWidth:0 }}>
        <button onClick={onMenuClick} className="lg:hidden" style={{ background:'none', border:'none', cursor:'pointer', color: isDark ? '#9ca3af' : '#6b7280', padding:6, display:'flex', flexShrink:0 }}>
          <Menu size={20} />
        </button>
...
/* ── App shell ───────────────────────────────────────────────────────────── */
function AppShell({ user, setUser, onLogout, isDark }: { user:any; setUser:(u:any)=>void; onLogout:()=>void; isDark:boolean }) {
  ...
  return (
    <ThemeContext.Provider value={{ theme, toggleTheme }}>
      {!user ? (
        ...
        <LanguageProvider>
          <Router>
            <AppShell user={user} setUser={setUser} onLogout={handleLogout} isDark={isDark} />
          </Router>
        </LanguageProvider>
      )}
