...
export default function Dashboard() {
  ...
  const fetchData = () => {
    ...
    ]).then(([financeData, membersData, loansData, savingsData]) => {
  ...
  const stats = isAdmin ? [
    ...
    },
    {
      ...
      value: `Rp ${(activeLoans / 1000000).toFixed(1)}jt`,
      change: '+12%',
      trend: 'up',
      icon: HandCoins,
    ...
    },
    {
      ...
      value: `Rp ${(totalSHU / 1000000).toFixed(1)}jt`,
      change: '+8%',
      trend: 'up',
      icon: Coins,
    ...
    { name: t('dashboard.my_savings'), value: `Rp ${(savingsList.filter(s => s.memberId === user?.id).reduce((sum, s) => sum + (s.type === 'Withdrawal' ? -(s.amount || 0) : (s.amount || 0)), 0) / 1000000).toFixed(2)}jt`, change: 'Total', trend: 'up', icon: Wallet, color: 'text-emerald-400', iconBg: 'bg-emerald-500/20', accent: '#10b981', label: 'Simpanan Anda' },
    { name: t('dashboard.active_loans'), value: `Rp ${(loans.filter(l => l.memberId === user?.id && l.status === 'approved').reduce((sum, l) => sum + (l.amount || 0), 0) / 1000000).toFixed(2)}jt`, change: 'Aktif', trend: 'down', icon: HandCoins, color: 'text-amber-400', iconBg: 'bg-amber-500/20', accent: '#f59e0b', label: 'Pinjaman berjalan' },
    { name: t('dashboard.shu_received'), value: `Rp ${((user?.total_shu || 0) / 1000000).toFixed(2)}jt`, change: 'Dividen', trend: 'up', icon: Coins, color: 'text-purple-400', iconBg: 'bg-purple-500/20', accent: '#a855f7', label: 'SHU Anda' },
        ...
        {stats.map((stat, i) => (
          <div key={stat.name} className={cn(card, "p-5 hover:border-opacity-20 transition-all group")}>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className={cn("text-[11px]", isDark ? "text-slate-500" : "text-slate-400")}>{stat.label}</span>
            </div>
          </div>
        ))}
      </div>
            ...
            <div className="h-[220px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
                  ...
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: isDark ? '#475569' : '#94a3b8', fontSize: 11 }} tickFormatter={(v) => `${v / 1000000}jt`} />
                  <Tooltip
                    contentStyle={{
              ...
              {[
                ...
                <div key={item.label}>
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <p className={cn("text-xs font-bold", item.color)}>Rp {(item.amount / 1000000).toFixed(1)}jt</p>
                  </div>
