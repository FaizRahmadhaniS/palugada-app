    ...
    <div className="p-4 lg:p-8 max-w-4xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex justify-end pt-4">
              <button 
                type="submit"
                disabled={loading}
                className="px-6 py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-200 dark:shadow-none flex items-center gap-2 disabled:opacity-50"
              >
                {loading ? (
      ...
      >
        ...
        <div className="p-6 lg:p-8 space-y-6">
          <TwoFactorSetup isEnabled={user?.is_2fa_enabled} onUpdate={onUpdate} />
        </div>
      </motion.div>
    </div>
  );
...
function TwoFactorSetup({ isEnabled, onUpdate }: { isEnabled?: boolean, onUpdate: (user: any) => void }) {
  ...
  if (isEnabled) {
    return (
      <div className="flex items-center justify-between p-4 bg-emerald-50 dark:bg-emerald-500/10 rounded-2xl border border-emerald-100 dark:border-emerald-900/30">
        <div>
        ...
        <div className="h-10 w-10 bg-emerald-100 dark:bg-emerald-800 rounded-full flex items-center justify-center text-emerald-600 dark:text-emerald-400">
          <CheckCircle size={20} />
        </div>
      </div>
    );
