...
app.use('/api', (req, res, next) => {
  ...
  const DEFAULT_EMAIL_TEMPLATES: EmailTemplateConfig[] = [
    { id: 'member_approved',    enabled: true  },
    { id: 'loan_approved',      enabled: true  },
    { id: 'payment_reminder',   enabled: false },
    { id: 'shu_distribution',   enabled: true  },
    { id: 'monthly_report',     enabled: false },
    { id: 'withdrawal_process', enabled: true  },
  ];
  ...
  function getEmailTransporter() {
    ...
    if (!cfg.gmailUser || !cfg.gmailAppPassword) return null;
  ...
  async function sendEmailViaSMTP(to: string, subject: string, html: string): Promise<{ success: boolean; message: string }> {
    const cfg = getEmailConfig();
    const transporter = getEmailTransporter();
    if (!transporter) {
      return { success: false, message: 'Email belum dikonfigurasi. Isi Gmail User dan App Password di Pengaturan Email.' };
    }
    try {
      await transporter.sendMail({
        from: `"${cfg.fromName || 'Koperasi Palugada'}" <${cfg.gmailUser}>`,
        to,
        subject,
        html,
      });
      ...
      return { success: false, message: `Gagal mengirim email: ${error.message}` };
    }
  }
  ...
  app.post("/api/withdrawals", async (req, res) => {
    ...
    try {
      ...
      // PENTING: sertakan SEMUA transaksi sukarela (deposit success + withdrawal success/pending)
      // agar saldo yang dihitung benar-benar mencerminkan sisa saldo yang bisa ditarik
      const { data: sukarelaTx } = await db.from('transactions')
        .select('type, amount, status')
        .eq('member_id', memberId)
        .eq('category', 'Savings')
        .ilike('description', '%sukarela%');

      const sukarelaSaldo = (sukarelaTx || []).reduce((sum: number, t: any) => {
        // Deposit yang sudah success → tambah saldo
        if (t.type !== 'Withdrawal' && t.status === 'success') return sum + (t.amount || 0);
        // Withdrawal yang sudah success ATAU masih pending → kurangi saldo (supaya tidak bisa ditarik dobel)
        if (t.type === 'Withdrawal' && (t.status === 'success' || t.status === 'pending')) return sum - (t.amount || 0);
        return sum;
      }, 0);
      ...
      // Build description dengan info bank — SELALU sertakan kata "Sukarela" di awal
      // agar transaksi ini terdeteksi konsisten oleh semua perhitungan saldo (frontend & backend)
      const bankInfo = bankName ? `Bank: ${bankName} | Rek: ${accountNumber} | A/N: ${accountHolder}` : '';
      const fullDescription = ['Penarikan Simpanan Sukarela', bankInfo, description].filter(Boolean).join(' | ');
  ...
  app.get("/api/savings/filter", async (req, res) => {
    // PENTING: Data simpanan tersimpan di tabel 'transactions' dengan category='Savings'
    // (sama seperti GET /api/savings), BUKAN di tabel terpisah 'savings'
    // CATATAN: kolom 'status' di tabel transactions ternyata smallint (bukan text seperti dokumentasi),
    // jadi filter status TIDAK dilakukan di query DB — difilter manual di JS setelah fetch.
    const { memberId, status, minAmount, maxAmount, search, sortBy = 'created_at', order = 'desc' } = req.query;
    try {
      let query = db.from('transactions').select('*').eq('category', 'Savings');
      ...
      const sortColumnMap: Record<string, string> = {
        name: 'created_at', amount: 'amount', date: 'created_at', created_at: 'created_at',
      };
      const sortCol = sortColumnMap[sortBy as string] || 'created_at';
      query = query.order(sortCol, { ascending: order === 'asc' });

      const { data: transactions, error } = await query;
      if (error) {
        console.error('savings/filter error:', error.message);
        return res.json([]);
      }

      // Resolve member names (sama seperti GET /api/savings)
      const { data: members } = await db.from('members').select('id, name');
      const memberMap = (members || []).reduce((acc: any, m: any) => { acc[m.id] = m.name; return acc; }, {});

      let mapped = (transactions || []).map((t: any) => ({
        id: t.id,
        member_id: t.member_id,
        member_name: memberMap[t.member_id] || t.member_name || 'Unknown',
        amount: t.amount,
        type: t.type,
        description: t.description,
        status: t.status,
      ...
      // Filter status di JS (status bisa berupa angka atau string tergantung skema asli)
      if (status) {
        const statusLower = String(status).toLowerCase();
        mapped = mapped.filter((m: any) => {
          const ms = String(m.status ?? '').toLowerCase();
          return ms === statusLower
            || (statusLower === 'success' && (ms === '1' || ms === 'true'))
            || (statusLower === 'pending' && (ms === '0' || ms === 'false'));
        });
      }
      ...
      if (search) {
        ...
        mapped = mapped.filter((m: any) => (m.member_name || '').toLowerCase().includes(s));
      }
  ...
  // GET loan payments — baca dari loan_repayments (tabel yang benar)
  app.get("/api/loan-payments/:loanId", async (req, res) => {
    const { loanId } = req.params;
    const user = (req.session as any).user;
    if (!user) return res.status(401).json({ message: 'Unauthorized' });
    try {
      const { data: repayments } = await db.from('loan_repayments')
        .select('*')
        .eq('loan_id', loanId)
        .order('payment_date', { ascending: false });
      const normalized = (repayments || []).map((r: any) => ({
        id: r.id,
        loan_id: r.loan_id,
        amount: r.amount_paid || r.amount || 0,
        payment_date: r.payment_date || r.created_date,
        notes: r.notes || '',
        status: r.status || 'completed',
        created_at: r.created_date || r.payment_date,
      }));
      res.json(normalized);
    } catch (error: any) {
      res.status(500).json({ message: error.message });
  ...
  // POST tambah pembayaran manual (admin)
  app.post("/api/loan-payments", async (req, res) => {
    const user = (req.session as any).user;
    if (!user || user.role !== 'admin') return res.status(403).json({ message: 'Forbidden' });
    const { loanId, amount, paymentDate, notes } = req.body;
    if (!loanId || !amount || Number(amount) <= 0) {
      return res.status(400).json({ success: false, message: 'Loan ID dan jumlah wajib diisi' });
    }
    try {
      const now = new Date().toISOString();
      const id = `RPY-${Date.now()}`;
      const { data: repayment, error: repError } = await db.from('loan_repayments')
        .insert({
          id, loan_id: loanId, amount_paid: Number(amount),
          payment_date: paymentDate || now.split('T')[0],
          notes: notes || '', status: 'completed',
          company_code: 'PALUGADA', created_by: user.id, created_date: now,
        }).select().single();
      if (repError) throw repError;
      const { data: loan } = await db.from('loans').select('remaining_balance, paid_amount, amount').eq('id', loanId).single();
      if (loan) {
        const newRemaining = Math.max(0, (loan.remaining_balance || loan.amount || 0) - Number(amount));
        const newPaid = (loan.paid_amount || 0) + Number(amount);
        await db.from('loans').update({
      ...
      res.json({ success: true, payment: { id: repayment?.id || id, loan_id: loanId, amount: Number(amount), payment_date: paymentDate || now.split('T')[0], notes: notes || '', status: 'completed' }});
    } catch (error: any) {
      res.status(500).json({ success: false, message: error.message });
    }
  });
  ...
  app.post("/api/shu/calculate", async (req, res) => {
    ...
    if (!period || !totalProfit || Number(totalProfit) <= 0) {
      return res.status(400).json({ success: false, message: 'Period dan total laba wajib diisi dan lebih dari 0' });
    }
    try {
      // Cek duplikat periode
      const { data: existing } = await db.from('shu_distributions').select('id').eq('period', period).limit(1);
      if (existing && existing.length > 0) {
        return res.status(409).json({ success: false, message: `SHU periode ${period} sudah pernah dihitung.` });
      }
      // Ambil semua anggota yang punya simpanan (tanpa filter status agar tidak salah)
      const { data: allMembers } = await db.from('members').select('id, name, total_savings, status');
      const members = (allMembers || []).filter((m: any) => {
        const s = String(m.status || '').toLowerCase();
        const hasActiveStatus = s === 'aktif' || s === 'active' || s === 'approved';
        return hasActiveStatus && (m.total_savings || 0) > 0;
      });
      const eligible = members.length > 0 ? members : (allMembers || []).filter((m: any) => (m.total_savings || 0) > 0);
      if (!eligible || eligible.length === 0) {
        return res.status(400).json({ success: false, message: 'Tidak ada anggota dengan simpanan yang dapat menerima SHU' });
      }
      const totalSavings = eligible.reduce((sum: number, m: any) => sum + (m.total_savings || 0), 0);
      const profitToDistribute = Number(totalProfit) * (Number(distributionRate) / 100);
      const distributions = eligible.map((m: any) => ({
        member_id: m.id,
        member_name: m.name,
        period,
      ...
      for (const dist of (result || [])) {
        const { data: md } = await db.from('members').select('total_shu').eq('id', dist.member_id).single();
        if (md) await db.from('members').update({ total_shu: (md.total_shu || 0) + dist.share_amount }).eq('id', dist.member_id);
      }
      res.json({ success: true, distributions: result, summary: { totalMembers: eligible.length, totalDistributed: profitToDistribute, period, distributionRate } });
    } catch (error: any) {
      res.status(500).json({ success: false, message: error.message });
    }
  });
  ...
  app.get("/api/reports/monthly", async (req, res) => {
    ...
    try {
      // 1. Simpanan masuk bulan ini
      let savingsData: any[] = [];
      try {
        const { data } = await db.from('savings').select('amount,status').gte('date', startDate).lt('date', endDate);
        savingsData = data || [];
      } catch (_) {}
      const totalSavings = savingsData
        .filter((s: any) => s.status !== 'pending' && s.status !== 0)
        .reduce((sum: number, s: any) => sum + (s.amount || 0), 0);

      // 2. Penarikan bulan ini
      let withdrawalData: any[] = [];
      try {
        const { data } = await db.from('transactions').select('amount').eq('type','Withdrawal').eq('status','success').gte('date', startDate).lt('date', endDate);
        withdrawalData = data || [];
      } catch (_) {}
      const totalWithdrawals = withdrawalData.reduce((sum: number, t: any) => sum + (t.amount || 0), 0);

      // 3. Pinjaman dicairkan bulan ini
      let loansData: any[] = [];
      try {
        const { data } = await db.from('loans').select('amount').gte('created_date', startDate).lt('created_date', endDate).in('status', ['approved','paid_off']);
        loansData = data || [];
      } catch (_) {}
      const totalLoans = loansData.reduce((sum: number, l: any) => sum + (l.amount || 0), 0);

      // 4. Angsuran diterima bulan ini
      let repayData: any[] = [];
      try {
        const { data } = await db.from('loan_repayments').select('amount_paid').gte('payment_date', startDate).lt('payment_date', endDate).eq('status','completed');
        repayData = data || [];
      } catch (_) {}
      const totalRepayments = repayData.reduce((sum: number, r: any) => sum + (r.amount_paid || 0), 0);

      // 5. Keuangan operasional bulan ini
      let financeData: any[] = [];
      try {
        const { data } = await db.from('finance').select('amount,type').gte('date', startDate).lt('date', endDate);
        financeData = data || [];
      } catch (_) {}
      const totalIncome  = financeData.filter((f: any) => f.type === 'Income') .reduce((sum: number, f: any) => sum + (f.amount || 0), 0);
      const totalExpense = financeData.filter((f: any) => f.type === 'Expense').reduce((sum: number, f: any) => sum + (f.amount || 0), 0);

      // 6. Anggota baru bulan ini
      let newMembersData: any[] = [];
      try {
        const { data } = await db.from('members').select('id').gte('join_date', startDate).lt('join_date', endDate);
        newMembersData = data || [];
      } catch (_) {}
      const newMembers = newMembersData.length;

      const transactionCount = savingsData.length + withdrawalData.length + repayData.length;
      const netCashflow = totalSavings + totalRepayments - totalLoans - totalWithdrawals;

      res.json({
        month, year,
        totalSavings, totalWithdrawals, totalLoans, totalRepayments,
        totalIncome, totalExpense, newMembers, transactionCount, netCashflow,
      });
    } catch (error: any) {
  ...
  app.get("/api/members/filter", async (req, res) => {
    ...
    try {
      ...
      const sortColumnMap: Record<string, string> = {
        name: 'name', total_savings: 'total_savings', join_date: 'join_date', email: 'email',
      };
      const sortCol = sortColumnMap[sortBy as string] || 'name';
      query = query.order(sortCol, { ascending: order === 'asc' });

      const { data: members, error } = await query;
      if (error) {
        console.error('members/filter error:', error.message);
        return res.json([]);
      }
      res.json(members || []);
    } catch (error: any) {
  ...
  app.get("/api/loans/filter", async (req, res) => {
    const { status, memberId, minAmount, maxAmount, search, sortBy = 'date', order = 'desc' } = req.query;
    try {
      ...
      const sortColumnMap: Record<string, string> = {
        name: 'member_name', amount: 'amount', date: 'date', created_at: 'created_date',
      };
      const sortCol = sortColumnMap[sortBy as string] || 'date';
      ...
      if (error) {
        console.error('loans/filter error:', error.message);
        return res.json([]);
      }
      res.json(loans || []);
    } catch (error: any) {
  ...
  app.get('/api/settings/email', (req, res) => {
    ...
    if (!user || user.role !== 'admin') return res.status(403).json({ message: 'Forbidden' });
    const cfg = getEmailConfig();
    // Jangan kirim app password penuh ke frontend — mask sebagian
    const masked = {
      ...cfg,
      gmailAppPassword: cfg.gmailAppPassword ? '••••••••••••••••' : '',
      hasAppPassword: !!cfg.gmailAppPassword,
    };
    res.json(masked);
  });
  ...
  app.post('/api/settings/email', (req, res) => {
    ...
    try {
      ...
      // Kalau field appPassword dikirim sebagai mask (titik-titik), pertahankan yang lama
      const merged: EmailConfig = {
        ...existing,
        ...incoming,
        gmailAppPassword: (incoming.gmailAppPassword && !/^•+$/.test(incoming.gmailAppPassword))
          ? incoming.gmailAppPassword
          : existing.gmailAppPassword,
      };
      fs.writeFileSync(EMAIL_CONFIG_FILE, JSON.stringify(merged, null, 2));
  ...
  // ── Kirim Email Test (benar-benar terkirim via Gmail SMTP) ─────────────────
    ...
    const result = await sendEmailViaSMTP(
      ...
      `<div style="font-family:Arial,sans-serif;padding:24px;background:#f8fafc">
        <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:12px;padding:32px;border:1px solid #e2e8f0">
          <h2 style="color:#059669;margin:0 0 16px">✓ Email Test Berhasil</h2>
          <p style="color:#334155;line-height:1.6">Ini adalah email test untuk memverifikasi integrasi email Anda di sistem Palugada.</p>
          <p style="color:#94a3b8;font-size:13px;margin-top:24px">Dikirim oleh ${user.name || 'Admin'} pada ${new Date().toLocaleString('id-ID')}</p>
        </div>
      </div>`
    );

    if (result.success) {
      res.json({ success: true, message: `Email test berhasil dikirim ke ${toEmail}` });
    } else {
      res.status(500).json({ success: false, message: result.message });
    }
  });

  // ── Kirim Email Notifikasi (dipakai oleh sistem untuk event seperti approval, dll) ──
  app.post("/api/notifications/send-email", async (req, res) => {
    const user = (req.session as any).user;
    if (!user || user.role !== 'admin') return res.status(403).json({ message: 'Forbidden' });

    const { userId, subject, message, template = 'generic', toEmail } = req.body;
    try {
      // Selalu buat notifikasi in-app
      await createNotification(userId, subject, message, 'email', '');

      // Cek apakah template ini diaktifkan di pengaturan
      const cfg = getEmailConfig();
      const templateCfg = cfg.templates.find(t => t.id === template);
      const templateEnabled = templateCfg ? templateCfg.enabled : true;

      if (!templateEnabled) {
        return res.json({ success: true, message: 'Notifikasi in-app dibuat (template email dinonaktifkan)', emailSent: false });
      }

      // Resolve email tujuan: pakai toEmail jika ada, atau cari dari users table via userId
      let targetEmail = toEmail;
      if (!targetEmail && userId) {
        const { data: targetUser } = await db.from('users').select('email').eq('id', userId).maybeSingle();
        targetEmail = targetUser?.email;
      }

      if (!targetEmail) {
        return res.json({ success: true, message: 'Notifikasi in-app dibuat (email tujuan tidak ditemukan)', emailSent: false });
      }
      ...
      const emailResult = await sendEmailViaSMTP(
        ...
        `<div style="font-family:Arial,sans-serif;padding:24px;background:#f8fafc">
          <div style="max-width:480px;margin:0 auto;background:#fff;border-radius:12px;padding:32px;border:1px solid #e2e8f0">
            <h2 style="color:#059669;margin:0 0 16px">${subject}</h2>
            <p style="color:#334155;line-height:1.6">${message}</p>
            <p style="color:#94a3b8;font-size:13px;margin-top:24px">Koperasi Palugada — ${new Date().toLocaleString('id-ID')}</p>
          </div>
        </div>`
      );
      ...
      res.json({
        success: true,
        message: emailResult.success ? 'Notifikasi & email terkirim' : `Notifikasi dibuat, namun email gagal: ${emailResult.message}`,
  ...
  // Handle 404 untuk API routes — di paling bawah setelah semua routes
  app.use("/api/*", (req, res) => {
